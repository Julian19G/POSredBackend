        ...
        <div class="card-body p-4">
            ...
            @if($corriendo && $url)
                <div class="d-flex align-items-center gap-2 mb-3">
                    ...
                    @if($desde)<span class="text-muted small">desde {{ $desde }}</span>@endif
                </div>

                <label class="form-label small fw-semibold">Link de la tienda</label>
                <div class="input-group mb-2">
                    <input type="text" id="tunnel-url" class="form-control" value="{{ $url }}" readonly>
                    <button type="button" class="btn btn-outline-primary" id="btn-copiar">Copiar</button>
                </div>
                <div class="small text-muted mb-3">
                    Link de referido: <code>{{ $url }}/&lt;codigo&gt;/register</code>
                </div>

                <form action="{{ route('tunnel.stop') }}" method="POST">
                    @csrf
            ...
            @else
                ...
                <form action="{{ route('tunnel.start') }}" method="POST">
                    @csrf
                    <button class="btn btn-success" {{ $tieneCloudflared ? '' : 'disabled' }}>▶ Activar túnel</button>
                </form>
            @endif
...
<script>
    ...
    (function () {
        const t = setInterval(function () {
            fetch('{{ route('tunnel.status') }}', { headers: { 'Accept': 'application/json' } })
