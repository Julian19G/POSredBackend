...
@section('content')
<div class="container">
    <h1>Editar Detalle de Venta #{{ $detalleVenta->id }}</h1>
    ...
    <form action="{{ route('detalle_ventas.update', $detalleVenta->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="number" name="cantidad" id="cantidad" class="form-control" min="1" value="{{ old('cantidad', $detalleVenta->cantidad) }}" required>
        </div>

        <div class="mb-3">
            <label for="precio_unitario" class="form-label">Precio Unitario</label>
            <input type="number" name="precio_unitario" id="precio_unitario" step="0.01" min="0" class="form-control" value="{{ old('precio_unitario', $detalleVenta->precio_unitario) }}" required>
        </div>

        <div class="mb-3">
            <label for="subtotal" class="form-label">Subtotal</label>
            <input type="number" name="subtotal" id="subtotal" step="0.01" min="0" class="form-control" value="{{ old('subtotal', $detalleVenta->subtotal) }}" required>
